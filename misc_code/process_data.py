...
def main(args):
    ...
    df = pd.read_csv(args.label, names=['path','label'], sep=' ')
    
    for _, row in tqdm(df.iterrows()):

        img,_=read_image(args.img_path + row['path'],len(row['label']),32,16)

        cv2.imwrite(args.output_folder_name+row['path'],img)
