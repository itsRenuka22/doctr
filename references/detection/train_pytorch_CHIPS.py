...
@torch.no_grad()
def evaluate_test(model, val_loader, batch_transforms, val_metric, output_dir, images_dir, amp=False):
    # Model in eval mode
    model.eval()
    # Reset val metric
    val_metric.reset()
    ...
    for images, targets, name in tqdm(val_loader):
        ...
        img = cv2.imread(images_dir + name[0])
        height, width, _ = img.shape
        df = pd.DataFrame(loc_preds[0]['words'], columns=['x1','y1','x2','y2','rot'])
        df['id'] = df.index + 1
        df['x1'] = df['x1'].apply(lambda x : int(x*width))
        df['x2'] = df['x2'].apply(lambda x : int(x*width))
        df['y1'] = df['y1'].apply(lambda x: int(x*height))
        df['y2'] = df['y2'].apply(lambda x: int(x*height))
        
        df = df[['id','x1','y1','x2','y2']]
        out_dir = f'{output_dir}/{name[0][:-4]}.txt'
        df.to_csv(out_dir, index=False, header=False, sep=' ')
...
def main(args):
    ...
    val_set = DetectionDataset(
        ...
        label_path=os.path.join(args.val_path, args.name+".json"),
    ...
    with open(os.path.join(args.val_path, args.name+".json"), "rb") as f:
    ...
    if args.test_only:
        ...
        output_dir = f'/data/BADRI/OCR/results/detection/txt/{args.name}/'
        images_dir = '/data/BADRI/OCR/data/CHIPS1/test/images/'
        
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        
        val_loss, recall, precision, mean_iou = evaluate_test(model, val_loader, batch_transforms, val_metric, output_dir, images_dir, amp=args.amp)
        print(
    ...
    train_set = DetectionDataset(
        ...
        label_path=os.path.join(args.train_path, args.name+".json"),
    ...
    with open(os.path.join(args.train_path, args.name+".json"), "rb") as f:
