...
def main(args):
    ...
    if isinstance(args.train_path, str):
        with open(os.path.join(args.train_path, args.val_json), "rb") as f:
            val_hash = hashlib.sha256(f.read()).hexdigest()

        val_set = RecognitionDataset(
            img_folder=os.path.join(args.train_path, args.images),
            labels_path=os.path.join(args.train_path, args.val_json),
    ...
    if isinstance(args.train_path, str):
        ...
        parts = (
            [base_path]
            if base_path.joinpath(args.train_json).is_file()
            else [base_path.joinpath(sub) for sub in os.listdir(base_path)]
        )
        with open(parts[0].joinpath(args.train_json), "rb") as f:
            train_hash = hashlib.sha256(f.read()).hexdigest()

        train_set = RecognitionDataset(
            parts[0].joinpath(args.images),
            parts[0].joinpath(args.train_json),
            img_transforms=Compose(
                [
                    T.Resize((args.input_size, 4 * args.input_size), preserve_aspect_ratio=True),
                    # Augmentations
        ...
        if len(parts) > 1:
            for subfolder in parts[1:]:
                train_set.merge_dataset(
                    RecognitionDataset(subfolder.joinpath(args.images), subfolder.joinpath(args.train_json))
                )
    else:
        train_hash = None
        # Load synthetic data generator
...
def parse_args():
    ...
    parser.add_argument("--images", type=str, default=None, help="path to train data folder(s)")
    parser.add_argument("--train_json", type=str, default=None, help="path to train data folder(s)")
    parser.add_argument("--val_json", type=str, default=None, help="path to train data folder(s)")
    parser.add_argument(
        "--train-samples",
        type=int,
        default=1000,
