...
def evaluate_test(model, val_loader, batch_transforms, val_metric, out_file, amp=False):
    ...
    df = pd.DataFrame(list(zip(actual, preds)), columns=['name', 'pred'])
    # print(df)
    df.to_csv(out_file + '.txt', sep=' ', index=False, header=False)
    val_loss /= batch_cnt
...
def main(args):
    ...
    if args.test_only:
        ...
        output_dir = '/data/BADRI/OCR/results/recognition/' + args.out_file + '/' + args.arch + '/' 
        ...
        val_loss, exact_match, partial_match = evaluate_test(model, val_loader, batch_transforms, val_metric, output_dir + args.vocab.split('_')[1], amp=args.amp)
